...
class Diffusion:
    def __init__(self, T=500, beta_start=1e-4, beta_end=0.02, diff_type='DDPM', img_height=560, img_width=870, device="cuda"):
        """
    ...
    def p_sample_loop(self, model, batch_size, timesteps_to_save=None, y=None, verbose=True):
        ...
        if timesteps_to_save is not None:
            intermediates = []
        with torch.no_grad():
            x = torch.randn((batch_size, 1, self.img_height, self.img_width)).to(self.device)
            for i in pbar:
                ...
                if timesteps_to_save is not None and i in timesteps_to_save:
                    ...
                    intermediates.extend(x_itermediate)
