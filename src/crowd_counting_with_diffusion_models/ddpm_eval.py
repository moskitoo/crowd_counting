...
class VGG(nn.Module):
    def __init__(self, num_classes=5):
        ...
        self.features = torchvision.models.vgg11(pretrained=True).features[:10]
...
def frechet_distance(mu1, sigma1, mu2, sigma2):
    ...
    # fid = linalg.sqrtm(linalg.norm(mu1 - mu2)**2 + np.trace(sigma1 + sigma2 - 2*linalg.sqrtm(sigma1*sigma2)))

    fid = np.square(np.abs(mu1 - mu2)).sum() + np.trace(sigma1 + sigma2 - 2*linalg.sqrtm(sigma1@sigma2))
...
if __name__ == '__main__':
    ...
    classifier.load_state_dict(torch.load('/zhome/d4/a/214319/advanced_dl_in_cv/week_5/ex6/weights/classifier/model.pth', map_location=device))

    unet_ddpm = UNet(device=device)
    unet_ddpm.eval()
    unet_ddpm.to(device)
    # unet_ddpm.load_state_dict(torch.load('weights/DDPM/model.pth', map_location=device))
    unet_ddpm.load_state_dict(torch.load('/zhome/d4/a/214319/advanced_dl_in_cv/week_5/ex6/weights/DDPM/model.pth', map_location=device))
    ...
    unet_ddpm_cFg.load_state_dict(torch.load('/zhome/d4/a/214319/advanced_dl_in_cv/week_5/ex6/weights/DDPM-cfg/model.pth', map_location=device))

    model = VGG()
    model.to(device)
    model.eval()
    # model.load_state_dict(torch.load('weights/vgg-sprites/model.pth', map_location=device))
    model.load_state_dict(torch.load('/zhome/d4/a/214319/advanced_dl_in_cv/week_5/ex6/weights/vgg-sprites/model.pth', map_location=device))
    ...
    with open('fid_scores_150epochs.txt', 'w') as f:
        f.write(f'FID classifier guidance: {fid_cg:.3f}\n')
        f.write(f'FID classifier-free guidance: {fid_cFg:.3f}\n')
